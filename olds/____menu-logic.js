...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showContextMenu(brickPage) {
    const contextContainer = document.getElementById('contextMenu');
    if (!contextContainer || !contextMenus) return;

    const brickKey = brickPage.replace('.html', '');
    const menuItems = contextMenus[brickKey];
    ...
    contextContainer.innerHTML = '<ul>' +
      menuItems.map(item => `<li><a href="${item.link}" target="_blank">${item.label}</a></li>`).join('') +
      '</ul>';
  }
  ...
  function loadFamily(familyKey) {
    ...
    brickList.querySelectorAll('li').forEach(li => {
      li.addEventListener('click', () => {
        ...
        showContextMenu(li.dataset.page);  // 🔥 Affiche le menu contextuel !
      });
    });
  }
    ...
    searchBox.addEventListener('input', (e) => {
      ...
      brickList.querySelectorAll('li').forEach(li => {
        li.addEventListener('click', () => {
          ...
          showContextMenu(li.dataset.page);
        });
      });
    });
  }
